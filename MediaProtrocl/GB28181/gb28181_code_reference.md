...
- 媒体接收：RTP 头解析、payload 识别(PS/裸H.264/H.264 FU-A/H.265 FU)、PS 拆层、**FU 重组状态机**（重排序窗+超时+丢包/乱序/重复处理，H.264/H.265 共用，差异在头重建）
...
- **Alarm/MobilePosition 命令**（同模式扩展）：`gb28181_build_message_alarm`（根 `<Notify>` + `<CmdType>Alarm`，带 AlarmMethod/AlarmType/AlarmTime/AlarmDescription）；`gb28181_build_message_mobile_position`（根 `<Notify>` + `<CmdType>MobilePosition`，带 Longitude/Latitude/Speed/Direction/Altitude）。两者和 Keepalive 同模式（`<Notify>` 单向通知），差别只在 `<CmdType>` 和字段——印证"一个壳 + N 种 CmdType"的设计。
- **RTP over TCP 承载**（mock 作 server + 设备作 client）：`gb28181_start` 加 TCP 分支——自建 TCP socket connect 到平台 `remote_rtp_ip:port`，把已连接 fd 交给 jrtplib `RTPTCPTransmitter`（`RTPTCPAddress(sock)`）；SDP 写 `TCP/RTP/AVP` + `a=setup:active` + `a=connection:new`。mock 端加 `tcp_listen_sock`（bind 30000 listen，与 UDP 30000 同端口协议独立不冲突）+ `accept` 收 RTP。**TCP 流重组**：mock recv 按 RFC 4571 framing（2字节length前缀+RTP）切分，残留缓冲处理半包。已验证：stateful TCP connect 成功，mock accept 后收到 151 个完整 RTP（0 半包），PS 拆层正常。命令行第三参数 `tcp` 启用。**passive 模式**：config 加 `tcp_passive` 字段，SDP 写 `a=setup:passive`（设备作 server），`gb28181_start` 加 passive 分支做 listen（accept 留事件循环接入，当前 TODO）。
...
第 10 节是给有基础的人（6 阶段，假设懂网络协议）。本章给**纯小白**——从"不懂协议是什么"到"看懂代码"，8 个阶段。

### 阶段 0：先建立"协议是什么"的概念（30 分钟）

小白最容易卡在这里——直接看 SIP/RTP 字段会懵。先理解：

```
两台机器要通信，得先约定"怎么说话"——这就是协议。
协议规定：报文长什么样、按什么顺序发、出了错怎么办。
```

GB28181 不是单一协议，是**一组协议的组合**：
- SIP（信令，负责"谁和谁建会话"）
- SDP（媒体描述，负责"怎么传媒体"）
- RTP（媒体传输，负责"真正传视频字节"）
- MANSCDP（XML 命令，负责"设备控制"）

**先记住一句话**：GB28181 = SIP 谈事 + SDP 描述媒体 + RTP 传媒体 + XML 控制设备。

### 阶段 1：先跑起来看效果（15 分钟，不看代码）

**目标**：用最简单的方式看到"协议在工作"，建立感性认识。

1. 构建（按 README 的 cmake 命令）
2. 开两个窗口：先跑 `gb28181_sip_mock_server.exe`，再跑 `gb28181_device_stateful.exe`
3. **盯着控制台输出看**，不读代码

看 `REGISTER` → `401` → `REGISTER+Auth` → `200` → `INVITE` → `200+SDP` → `ACK` → RTP → `BYE`。每一行 `===== TX/RX =====` 就是一次协议交互。先看到"它在工作"，再理解"为什么这样工作"。

### 阶段 2：理解 SIP 报文长什么样（20 分钟）

从日志里挑一条 REGISTER，对照认字段：

```
REGISTER sip:127.0.0.1 SIP/2.0     ← 请求行（方法+目标）
Via: SIP/2.0/UDP ...                ← 经过的路径
From: <sip:...>;tag=...             ← 发起方
To: <sip:...>                       ← 接收方
Call-ID: ...                        ← 会话标识
CSeq: 1 REGISTER                    ← 序号+方法
Content-Length: 0                   ← body 长度
```

小白认知：SIP 报文就是一段文本（和 HTTP 请求长得像），`From/To/Call-ID/CSeq` 四件套定义"谁和谁在说哪句话"。不用背字段，先认"这是文本协议，有头有 body"。

对应：读 [`../gb28181_study.md`](../gb28181_study.md) 第 3 节（SIP 是什么）。

### 阶段 3：理解"会话建立"的全流程（30 分钟）

**目标**：理解 REGISTER→INVITE→ACK→RTP→BYE 这个顺序为什么这样。

读 `gb28181_study.md` 第 2 节（GB28181 管什么）。

小白认知（先理解顺序，不记字段）：
- REGISTER = "我上线了"（设备注册到平台）
- 401 = "你是谁，证明一下"（要求鉴权）
- INVITE = "我想看你的视频"（建媒体会话）
- 200 OK = "好的"（接受）
- ACK = "确认"（三步握手完成）
- RTP = "视频来了"（媒体传输）
- BYE = "结束了"（结束会话）

### 阶段 4：啃 Digest 鉴权（45 分钟，第一个难点）

**目标**：理解 401 之后设备怎么证明自己。

读 `gb28181_study.md` 第 3.1 节（含三次 MD5 辨析）。

本质：平台问"你是谁"（发 nonce），设备用密码算 MD5 证明自己（发 response），平台用同样密码重算验证。不传密码本身，传"用密码算出来的证明"——防密码被截获。三次 MD5 分别防：密码明文存、响应被剪贴重放、nonce 被重放。代码里 `nc/cnonce` 固定是学习简化，生产要递增。

### 阶段 5：啃 MESSAGE + XML 命令（45 分钟，核心认知）

**目标**：理解 Keepalive/Catalog/DeviceInfo/DeviceStatus 等"不是一个一个的协议，而是同一个壳换内容"。**这是最重要的认知突破。**

读 `gb28181_study.md` 第 3.2-3.4 节 + 第 3 节开头的"两层结构"图。

小白认知：SIP 的 MESSAGE 方法是"通用容器"（像 HTTP 的 POST），body 里放 XML，`<CmdType>` 决定是哪种命令。Keepalive/Catalog/DeviceInfo/DeviceStatus/Alarm/MobilePosition 都是同一个 MESSAGE 壳，差别只在 XML 的 `<CmdType>`。

**一旦理解这点，8 种命令不是一个一个学，而是学一个壳 + 换字符串。**

### 阶段 6：理解媒体层（60 分钟）

**目标**：理解"视频字节怎么从设备传到平台"。

读 `gb28181_study.md` 第 4-6 节 + 第 10 节（PS over RTP + 三层切片）。

小白认知（分三层，从外到内）：
1. **RTP**：视频字节装进 RTP 包发出去（加头：序号/时间戳/SSRC）
2. **PS**：视频帧先装进 PS 容器（像把照片装进相框），再装进 RTP
3. **编码**：PS 里面装的是 H.264/H.265 编码数据

关键区分（三层切片，别混）：
- NALU 层（语义切片）：切裸 H.264 NALU，理解 NALU 头
- PS 封装层（可选分片）：切帧→多 PS pack，当前代码不做
- RTP 分片层（通用字节切片）：切 PS pack→多 RTP，不理解内容

### 阶段 7：读代码（60 分钟，最后做）

**目标**：把协议知识对应到代码。

读本文档第 8-10 节 + 三个核心文件顶部导读。按这个顺序：

1. `gb28181_module.h`（先看接口，不进实现）
2. `gb28181_module.cpp` 第 3 层 MESSAGE 命令（`build_xml_message` + 各 `build_message_*`）——印证"一个壳换 CmdType"
3. `gb28181_module.cpp` 第 4 层 RTP/PS（`send_rtp_packet`/`send_rtp_payload_fragmented`/`send_h264_fu_a`）——印证三层切片
4. `gb28181_sip_mock_server.cpp` 主循环——看平台怎么收发
5. `gb28181_device_stateful.cpp` 状态机——看设备怎么常驻（第 9 节导读）

### 小白学习的关键原则

1. **先看现象再读代码**：先跑起来看日志，不要一上来读代码（会懵）
2. **先理解顺序再记字段**：`REGISTER→401→INVITE→ACK→RTP→BYE` 的顺序比每个字段重要
3. **先理解"一个壳"再学各命令**：MESSAGE+XML 是一个壳换 CmdType，理解了这点 8 种命令一起通
4. **先理解分层再记细节**：SIP 谈事/SDP 描述/RTP 传媒体/XML 控制——四层各管各的
5. **不追求一次全懂**：协议学习是反复的过程，先跑通再看细节

### 一天学习计划

- 上午：阶段 0-3（概念 + 跑起来 + 认报文 + 会话流程）
- 下午：阶段 4-5（Digest + MESSAGE 一个壳）——两个核心难点
- 晚上：阶段 6-7（媒体层 + 读代码）——巩固
